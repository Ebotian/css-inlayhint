...
##### 红灯测试
*   `test/semanticCollector/collectsDeclarationCandidatesFromStylesheet`
    *   输入一段包含普通规则的 CSS，必须能收集到每个 `Property: Value` 声明候选，并保留属性名、值文本和范围。
    *   参考 clangd：`InlayHintVisitor`, `VisitCallExpr`
*   `test/semanticCollector/ignoresNonDeclarationNodes`
    *   输入 `@media`、注释、空块等节点时，只能返回 declaration 候选，不能把非声明节点误识别为 hint 候选。
    *   参考 clangd：`TraverseDecl`
*   `test/semanticCollector/preservesShorthandValueTokenOrder`
    *   对 `margin: 10px 20px` 这类简写，必须按源码顺序保留值 token，不得在采集层重排或合并语义。
    *   参考 clangd：`processCall`, `chooseParameterNames`
*   `test/semanticCollector/keepsVariableReferencesAsRawCandidates`
    *   遇到 `var(--x)` 时，采集层只标记为原始候选，不得在这一层直接抑制、展开或补全变量语义。
    *   参考 clangd：`VisitCallExpr`
